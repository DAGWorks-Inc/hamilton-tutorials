"""
WORK IN PROGRESS CODE. WILL BE SHIPPED AS PART OF HAMILTON IN A FUTURE RELEASE.
"""
...
class HamiltonMagics(Magics):
    ...
    @argument(
        "-m", "--module_name", help="Optional module name."
    ...
    def with_functions(self, line, cell):
        ...
        if self.shell.user_ns.get("dr"):
            if not isinstance(self.shell.user_ns.get("dr"), driver.Driver):
                raise KeyError(
                    "The variable `dr` is already defined and is not a `driver.Driver`"
                )
...
def import_dataflow(
    ...
    if user:
        local_file_path = (
            USER_PATH.format(commit_ish=version, user=user, dataflow=dataflow)
            + "/__init__.py"
        )
    else:
        local_file_path = (
